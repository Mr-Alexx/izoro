...
import { Space, Popconfirm, Tag, Button, message, Avatar, Badge, Select } from 'antd';
...
import { account, remove, role } from './service';
...
const YangTao: FC = () => {
  ...
  const columns: ProColumns<TableListItem>[] = [
    ...
    },
    {
      title: '历史采购量',
      width: 150,
      dataIndex: 'history_purchase_quantity',
      key: 'history_purchase_quantity',
      align: 'center',
      search: false,
    },
    {
    ...
    },
    {
      ...
      render: (_, row, index, action) => [
        <Link key="view" to={`/product-develop/check-goods?id=${row.id}`}>
          查看
        </Link>,
        <Link key="edit" to={`/product-develop/yangtao/create?id=${row.id}&cate=2`}>
          编辑
        </Link>,
      ],
    },
  ...
  // 初始化自定义列
  useEffect(() => {
    getPageSettings('yangtao').then((data: Record<string, any>) => {
      setColumnsStateMap(data);
    });
  }, []);

  // useRequest(getPageSettings, {
  //   onSuccess: data => {
  //     // console.log(22, data);
  //     setColumnsStateMap(data);
  //   },
  ...
  return (
    <PageContainer>
      <ProTable<TableListItem, TableListPagination>
        ...
        scroll={{ x: 1300 }}
        toolBarRender={() => [
          <Link key="create" to="/product-develop/yangtao/create">
